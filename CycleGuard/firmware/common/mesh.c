/*
 * CycleGuard Sub-GHz TDMA mesh
 * Coordinator (Hub) assigns slots; nodes sleep between their slot and beacon.
 * Used for Smart Lock communication (long-range, low-frequency).
 */
#include "mesh.h"
#include <string.h>
...
{
    /* Slot 0 for first non-coordinator node (Smart Lock); rest reserved. */
...
void mesh_remove_stale(mesh_state_t *state, uint32_t now_ms)
{
    for (int i = state->node_count - 1; i >= 0; i--) {
        if (now_ms - state->nodes[i].last_seen_ms > 120000) {  /* 2 min timeout */
