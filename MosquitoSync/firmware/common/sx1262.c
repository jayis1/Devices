/*
 * MosquitoSync — SX1262 Radio Driver (Implementation)
...
static const ms_spi_interface_t *g_spi = NULL;
static ms_radio_state_t g_state = MS_RADIO_SLEEP;
...
int ms_radio_init(const ms_spi_interface_t *spi, const ms_radio_config_t *cfg)
{
    ...
    g_spi->transfer(0x02); /* Ramp time 200 us */
    g_spi->cs_release();
...
int ms_radio_rx(uint8_t *buf, uint8_t max_len, uint32_t timeout_ms)
{
    ...
    /* Set RX with timeout */
    uint32_t timeout = timeout_ms * 64; /* SX1262 timeout unit = 15.625us */
    g_spi->cs_select();
