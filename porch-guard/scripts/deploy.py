#!/usr/bin/env python3
"""
PorchGuard — Deployment Script

Sets up the PorchGuard cloud dashboard (FastAPI + Mosquitto MQTT + optional Postgres).
...
def deploy_local():
    """Deploy PorchGuard dashboard locally using Docker Compose."""
    dashboard_dir = os.path.join(os.path.dirname(__file__), "..", "software", "dashboard")

    print("🚀 Deploying PorchGuard locally with Docker Compose...")
    print("=" * 50)
    ...
    # Tables are auto-created by FastAPI startup

    print("\n" + "=" * 50)
    print("✅ PorchGuard is running!")
    print()
    ...
    print("  🚨 Pirate watch: Camera monitors 24/7")
    print("  📦 Deliveries: Tracked + clips in dashboard")
    print("  🔑 Courier codes: Issue from app or API")
    print("=" * 50)


def deploy_cloud():
    """Deploy PorchGuard to cloud (AWS ECS or GCP Cloud Run)."""
    ...
    print("   8. Configure push notifications (APNs/FCM) for pirate alerts")
    print("   9. Configure SMS gateway (Twilio) for EMERGENCY pirate alerts")
    print("  10. Set up S3/GCS bucket for clip storage (event clips)")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PorchGuard Deployment")
