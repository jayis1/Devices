...
Open `care_hub.kicad_pro` in KiCad 7+. Schematic sheets:
1. `care_hub MCU.sch` — ESP32-S3 + decoupling + flash
2. `care_hub Power.sch` — USB-C, MCP73871, TPS25940, LDO, LiPo
3. `care_hub SubGHz.sch` — SX1262 + SMA + matching network
4. `care_hub Cellular.sch` — SIM7000A + SIM + SMA
5. `care_hub Peripherals.sch` — BME280, DS3231, microSD, LEDs, buzzer, strobe