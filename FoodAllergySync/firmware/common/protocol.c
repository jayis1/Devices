...
int fas_encode_frame(fas_frame_t *frame, uint8_t *buffer, size_t buffer_size) {
    if (!frame || !buffer || frame->payload_length > FAS_MAX_PAYLOAD) {
        return -1;
    }
    ...
    frame->crc16 = fas_crc16_ccitt(buffer, 11 + frame->payload_length);
...
int fas_decode_frame(fas_frame_t *frame, const uint8_t *buffer, size_t length) {
    ...
    if (buffer[0] != FAS_PREAMBLE || buffer[1] != FAS_VERSION) {
        return -2;
    }

    uint8_t payload_length = buffer[10];
    if (payload_length > FAS_MAX_PAYLOAD || length != (size_t)(11 + payload_length + 2)) {
        return -3;
    }

    uint16_t expected = fas_crc16_ccitt(buffer, 11 + payload_length);
