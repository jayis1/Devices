# FreshKeep fridge-node Schematic

## KiCad Project

Open `fridge-node.kicad_pro` in KiCad 8+ to view/edit the schematic.
...
- `fridge-node.kicad_sch` — Main schematic
- `fridge-node.kicad_pcb` — PCB layout (work in progress)
- `fridge-node.kicad_pro` — KiCad project file

See BOM in `hardware/bom/fridge-node-bom.csv` for component details.
