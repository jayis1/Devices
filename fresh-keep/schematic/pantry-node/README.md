# FreshKeep pantry-node Schematic

## KiCad Project

Open `pantry-node.kicad_pro` in KiCad 8+ to view/edit the schematic.
...
- `pantry-node.kicad_sch` — Main schematic
- `pantry-node.kicad_pcb` — PCB layout (work in progress)
- `pantry-node.kicad_pro` — KiCad project file

See BOM in `hardware/bom/pantry-node-bom.csv` for component details.
