# Hub Node Schematic — SoleGuard

## MCU Architecture

Dual-MCU: **RP2040** (main compute + ML + display) + **ESP32-C6** (WiFi6/BLE5.3 + cloud bridge) + **nRF52840** (BLE mesh radio to body nodes).
...
```
  ...
  ESP32-C6 ───────  │  WiFi6 to cloud (MQTT/TLS)                   │
                    ...
                    │  UART to RP2040                              │
                    │                                              │
  Qi TX coil ─────  │  5W wireless charging pad for insoles+tag    │
                    └────────────────────────────────────────────┘
```
...
| GP11 | SPI0 MISO| shared SPI0 (SD card via MISO) |
...
*(KiCad project files generated from this spec — see hardware/bom/hub_node_bom.csv)*