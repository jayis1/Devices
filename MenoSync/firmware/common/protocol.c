/*
 * MenoSync — Protocol Implementation
...
{
    if (!out || payload_len > MS_MAX_PAYLOAD)
        return 0;
    ...
    uint16_t crc = ms_crc16(out, 8 + payload_len);
              ...
              ms_msg_header_t *hdr, uint8_t *payload, size_t payload_cap)
{
    ...
    /* Check sync bytes */
    if (in[0] != MS_SYNC0 || in[1] != MS_SYNC1)
        return -1;
    ...
    /* Verify CRC */
    uint16_t crc_calc = ms_crc16(in, 8 + payload_len);
