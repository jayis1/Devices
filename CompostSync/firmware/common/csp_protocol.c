/*
 * CompostSync Protocol implementation
 * csp_protocol.c
 */
...
/* Simplified AES-128-CCM (uses external AES; here we use a minimal stub).
 * In production, use a proper AES library (e.g., tinyAES or mbedTLS).
 * For this open-source reference implementation, we provide the structure
 * with a basic XOR-based integrity check as a placeholder. Real deployments
 * MUST use a proper AES-128-CCM implementation.
 */

/* Placeholder: simple obfuscation + CRC for demo.
 * Replace with mbedTLS aes_ccm_encrypt / aes_ccm_decrypt in production. */
static void csp_encrypt(const uint8_t *key, const uint8_t *nonce, size_t nonce_len,
                         const uint8_t *plaintext, size_t pt_len,
                         uint8_t *ciphertext, uint8_t *mac, size_t mac_len)
{
    /* XOR stream cipher placeholder (NOT SECURE — replace with AES-128-CCM) */
    uint8_t stream = 0;
    ...
    /* Fake MAC: truncated CRC of ciphertext + key */
    uint16_t fake_mac = csp_crc16(ciphertext, pt_len);
    fake_mac ^= csp_crc16(key, AES_KEY_SIZE);
...
{
    csp_header_t hdr;
    uint8_t encrypted[CSP_MAX_PAYLOAD + AES_MAC_SIZE];
    uint8_t nonce[AES_NONCE_SIZE];

    if (payload_len > CSP_MAX_PAYLOAD)
        return -1;
    ...
    hdr.src_id      = src;
    hdr.dst_id      = dst;
    hdr.timestamp   = 0; /* Set by caller or radio layer */

    /* Build nonce: src(2) + seq(1) + pad(9) */
    nonce[0] = (uint8_t)(src >> 8);
    nonce[1] = (uint8_t)(src & 0xFF);
    nonce[2] = seq;
    memset(nonce + 3, 0x34, 9);

    /* Encrypt payload + generate MAC */
    csp_encrypt(aes_key, nonce, AES_NONCE_SIZE,
                payload, payload_len, encrypted, encrypted + payload_len, AES_MAC_SIZE);

    /* Assemble: header + encrypted_payload + MAC + CRC */
    size_t offset = 0;
    ...
    uint16_t crc = csp_crc16(buf, offset);
...
{
    if (len < sizeof(csp_header_t) + CSP_CRC_SIZE + AES_MAC_SIZE)
        return -1;

    memcpy(header, buf, sizeof(csp_header_t));

    if (header->payload_len > CSP_MAX_PAYLOAD)
        return -1;

    /* Verify CRC */
    size_t total = sizeof(csp_header_t) + header->payload_len + AES_MAC_SIZE + CSP_CRC_SIZE;
    if (len < total)
        return -1;

    uint16_t crc_recv = (buf[total - 2] << 8) | buf[total - 1];
    uint16_t crc_calc = csp_crc16(buf, total - 2);
    ...
    memcpy(mac, buf + sizeof(csp_header_t) + header->payload_len, AES_MAC_SIZE);

    /* Decrypt in-place to output */
    uint8_t stream = 0;
    ...
    for (size_t i = 0; i < header->payload_len; i++) {
        payload[i] = buf[sizeof(csp_header_t) + i] ^ stream ^ (uint8_t)(i * 0x37 + 0xA5);
        stream = (stream << 1) | (stream >> 7);
    }
...
{
    switch (phase) {
        case PHASE_MESOPHILIC:  return "Mesophilic";
        case PHASE_THERMOPHILIC:return "Thermophilic";
        case PHASE_COOLING:     return "Cooling";
        case PHASE_MATURATION:  return "Maturation";
        case PHASE_CURED:       return "Cured";
