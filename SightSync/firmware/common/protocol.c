/**
 * SightSync Shared Communication Protocol — Implementation
 *
 * License: MIT
 */
...
{
    if (buf == NULL || payload_len > SS_MAX_PAYLOAD) {
        return 0;
    }

    uint8_t total = SS_HEADER_LEN + payload_len;
    if (buf_len < total) {
        return 0;
    }

    buf[0]  = SS_SYNC_BYTE_0;
    buf[1]  = SS_SYNC_BYTE_1;
    buf[2]  = SS_PROTOCOL_VERSION;
    ...
    buf[10] = sightsync_checksum(buf, 10);
...
{
    if (buf == NULL || buf_len < SS_HEADER_LEN) {
        return false;
    }

    if (!sightsync_validate(buf, buf_len)) {
        return false;
    }
...
bool sightsync_validate(const uint8_t *buf, uint8_t buf_len)
{
    ...
    if (buf[0] != SS_SYNC_BYTE_0 || buf[1] != SS_SYNC_BYTE_1) {
        return false;
    }

    if (buf[2] != SS_PROTOCOL_VERSION) {
        return false;
    }

    uint8_t payload_len = buf[9];
    if (buf_len < (uint8_t)(SS_HEADER_LEN + payload_len)) {
        return false;
    }

    uint8_t computed = sightsync_checksum(buf, 10);
    if (computed != buf[10]) {
        return false;
    }
