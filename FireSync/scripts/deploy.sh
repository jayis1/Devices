#!/bin/bash
# FireSync — Cloud Deployment Script
...
echo "╔══════════════════════════════════════════╗"
echo "║  FireSync Cloud Deployment               ║"
echo "╚══════════════════════════════════════════╝"
...
services:
  api:
    ...
    ports:
      - "8080:8080"
    environment:
      - JWT_SECRET=${JWT_SECRET:-firesync-secret}
      - MQTT_BROKER=mosquitto:1883
      - POSTGRES_URL=postgresql://firesync:***@db:5432/firesync
  ...
  db:
    ...
    environment:
      - POSTGRES_DB=firesync
      - POSTGRES_USER=firesync
      - POSTGRES_PASSWORD=firesync
    volumes:
  ...
  influxdb:
    ...
    environment:
      ...
      - DOCKER_INFLUXDB_INIT_USERNAME=firesync
      - DOCKER_INFLUXDB_INIT_PASSWORD=firesync123
      - DOCKER_INFLUXDB_INIT_ORG=firesync
...
echo "Starting FireSync cloud services..."
cd "$DASHBOARD_DIR"
$COMPOSE up -d $BUILD_FLAG $DEV_FLAG

echo ""
echo "✓ FireSync cloud deployed!"
