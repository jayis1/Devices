/**
 * DriveSync Shared Communication Protocol — Implementation
 *
 * License: MIT
 */
...
{
    if (buf == NULL || payload_len > DS_MAX_PAYLOAD) {
        return 0;
    }

    uint8_t total = DS_HEADER_LEN + payload_len;
    if (buf_len < total) {
        return 0;
    }

    /* Build header in buffer */
    buf[0]  = DS_SYNC_BYTE_0;
    buf[1]  = DS_SYNC_BYTE_1;
    buf[2]  = DS_PROTOCOL_VERSION;
    ...
    buf[10] = drivesync_checksum(buf, 10);
...
{
    if (buf == NULL || buf_len < DS_HEADER_LEN) {
        return false;
    }

    if (!drivesync_validate(buf, buf_len)) {
        return false;
    }
...
bool drivesync_validate(const uint8_t *buf, uint8_t buf_len)
{
    ...
    /* Check sync bytes */
    if (buf[0] != DS_SYNC_BYTE_0 || buf[1] != DS_SYNC_BYTE_1) {
        return false;
    }

    /* Check version */
    if (buf[2] != DS_PROTOCOL_VERSION) {
        return false;
    }

    /* Check payload length fits in buffer */
    uint8_t payload_len = buf[9];
    if (buf_len < (uint8_t)(DS_HEADER_LEN + payload_len)) {
        return false;
    }

    /* Verify checksum (over bytes 0..9, compare with byte 10) */
    uint8_t computed = drivesync_checksum(buf, 10);
    if (computed != buf[10]) {
        return false;
    }
